...
export function Wizard({ onCancel, onFinish, quote }: WizardProps) {
  ...
  const runAiAnalysis = () => {
    ...
    setTimeout(() => {
      ...
      // v2.25 — Generar campos para el formato proforma a partir de lo detectado.
      // Esta IA rule-based es la misma del análisis arriba; cuando se migre a
      // un LLM (Anthropic API) estos 4 bloques deberían tomar `requirements`
      // como input directo y devolver texto natural enriquecido.
      ...
      // Solución: nombres de productos, máx 2 separados por "+"
      const solutionLine = productNames.slice(0, 2).join(' + ') || 'Solución a medida';

      // Alcance: top 3-4 módulos / beneficios clave
      const scopeTokens = moduleNames.slice(0, 3);
      const scopeLine = scopeTokens.length > 0
        ? scopeTokens.join(' · ')
        : (reasons[0] || 'Alcance personalizado según requerimientos');

      // Modalidad: inferir si hay recurring (hosting/suscripción) vs. llave en mano
      const hasRecurring = suggested.some((it) => {
        const p = products.find((x) => x.id === it.product_id);
        return p?.requires_recurring;
      });
      const modalityLine = hasRecurring
        ? `Implementación + servicio recurrente · ${Math.max(2, Math.round(weeks * 0.7))} semanas`
        : `Implementación llave en mano · ${Math.max(2, Math.round(weeks * 0.7))} semanas`;

      // Justificación: narrativa 2 párrafos (paragraph 1 = contexto + propuesta,
      // paragraph 2 = complementos/recurring si aplica)
      ...
      const complementP = hasRecurring
        ? `El paquete se complementa con servicios recurrentes (hosting, renovaciones o soporte) que garantizan continuidad operativa sin interrupciones. La implementación está planificada para ${Math.max(
            2,
            Math.round(weeks * 0.7),
          )} semanas, con entregables validados en cada hito.`
        : `La implementación está planificada para ${Math.max(
            2,
            Math.round(weeks * 0.7),
          )} semanas, entregada bajo modalidad llave en mano con código fuente, documentación técnica y transferencia de conocimiento al equipo del cliente.`;

      setSolutionSummary(solutionLine);
      setScopeSummary(scopeLine);
      setModalitySummary(modalityLine);
      setJustificationText(`${contextP}\n\n${complementP}`);
      ...
      >
        ...
        <div className="nx-field">
          ...
          <input
            ...
            onChange={(e) => setClient({ ...client, contact_role: e.target.value })}
          />
        </div>
        <div className="nx-field">
        ...
        <div className="nx-field" style={{ gridColumn: '1 / -1' }}>
          <label className="nx-label">Dirección (opcional)</label>
          <input
            className="nx-input"
