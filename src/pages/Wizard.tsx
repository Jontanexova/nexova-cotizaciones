...
export function Wizard({ onCancel, onFinish }: WizardProps) {
  ...
  const [paymentTerms, setPaymentTerms] = useState('50% adelanto / 50% a la entrega');
  ...
  const submit = async () => {
    ...
    try {
      ...
      console.error(e);
      setSubmitError(e?.message || 'Error al crear la cotización');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <button className="btn btn-primary btn-lg" onClick={onSubmit} disabled={!canSubmit}>
          {submitting ? (
            ...
            <>
              Generar cotización <Icon name="check" size={14} />
            </>
          )}
        </button>
      </div>
