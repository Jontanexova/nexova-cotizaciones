...
import { Avatar, Loading, Modal, Stat, Toast, Topbar } from '../components/UI';
...
type SettingsTab = 'profile' | 'password' | 'organization' | 'branding' | 'exchange' | 'smtp' | 'integrations' | 'domains' | 'cleanup';
...
export function Settings() {
  ...
  const tabs: { key: SettingsTab; label: string; icon: any; show: boolean }[] = [
    ...
    // v2.24: Branding — exclusivo super_admin (rebrand completo de la plataforma)
    { key: 'branding', label: 'Branding', icon: 'sparkle', show: isSuperAdmin },
    { key: 'exchange', label: 'TC', icon: 'dollar', show: true },
    { key: 'smtp', label: 'SMTP Server', icon: 'mail', show: isAdmin },
    // v2.22: Integraciones — exclusivo super_admin (implica costos de APIs externas)
    { key: 'integrations', label: 'Integraciones', icon: 'sparkle', show: isSuperAdmin },
    // v2.21: Dominios permitidos — exclusivo para super_admin
    { key: 'domains', label: 'Dominios permitidos', icon: 'filter', show: isSuperAdmin },
    // v2.23: Limpieza — acciones destructivas, solo super_admin
    { key: 'cleanup', label: 'Limpieza', icon: 'trash', show: isSuperAdmin },
  ];
    ...
    <div className="fade-in">
      ...
      <div style={{ padding: '24px 32px', maxWidth: 820 }}>
        ...
        {activeTab === 'branding' && isSuperAdmin && (
          <BrandingTab onSaved={() => showToast('✓ Branding actualizado')} />
        )}
...
function OrganizationTab({ onSaved }: { onSaved: () => void }) {
  ...
  const dirty =
    ...
    (form.name !== (org.name || '') ||
      ...
      form.default_terms !== (org.default_terms || ''));
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div className="nx-field" style={{ gridColumn: '1 / -1' }}>
          <label className="nx-label">
            Notas y condiciones por defecto
            <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--ink-500)', fontWeight: 400 }}>
              Una línea por nota. Aparecen en todas las cotizaciones — el vendedor puede personalizarlas por cotización desde el Preview.
            </span>
          </label>
          <textarea
            className="nx-input"
            ...
            placeholder={
              'Los precios incluyen IGV (18%). El desglose se muestra en la tabla de totales.\n' +
              'El cliente proporcionará los contenidos (textos, imágenes, logo) en formato digital editable.\n' +
              'Se incluyen hasta 2 rondas de revisiones sobre el diseño. Cambios adicionales se cotizarán por separado.\n' +
              'La propuesta se activa con la firma de conformidad y el pago del adelanto acordado.\n' +
              'Los cargos recurrentes (hosting, renovaciones) no están incluidos en el total de la inversión inicial.'
            }
...
// v2.22: simplificado — solo TC manual. La configuración de API automática
// fue movida a la pestaña "Integraciones" (solo super_admin).
function ExchangeRateTab({ onSaved }: { onSaved: () => void }) {
  ...
  const lastUpdatedText = org?.exchange_rate_updated_at
    ...
    : 'Aún no se ha configurado un TC.';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {/* Card 1: Tipo de cambio actual (manual) */}
      <div className="nx-card nx-card-padded">
        <div
          style={{
            display: 'flex',
...
// ─── Tab: Integraciones (v2.22) ───
// Solo visible para super_admin. Gestiona conexiones con servicios externos
// que pueden implicar costos (API de SUNAT, futuras integraciones: SMTP externo,
// webhooks, CRM, etc.).
...
function IntegrationsTab({ onSaved }: { onSaved: () => void }) {
  const [org, setOrg] = useState<OrganizationSettings | null>(null);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  ...
  const saveApiConfig = async () => {
    ...
    try {
      ...
      if (providerDirty) {
        ...
        patch.exchange_rate_api_auth_scheme = authScheme;
  ...
  const syncNow = async () => {
    if (apiKeyDirty || autoSyncDirty || providerDirty) {
      setSyncResult({ ok: false, msg: 'Guarda primero los cambios antes de sincronizar.' });
      return;
    }
    if (!org?.peruapi_key) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      >
        <Icon name="info" size={14} style={{ flexShrink: 0, marginTop: 2 }} />
        <div>
          <div style={{ fontWeight: 700, marginBottom: 4 }}>Integraciones con servicios externos</div>
          Estas configuraciones conectan la plataforma con APIs de terceros que pueden generar
          costos adicionales según el plan contratado con cada proveedor. Por seguridad, esta
          sección solo es accesible al <strong>Super Admin</strong>.
        </div>
      </div>

      {/* Card: Sincronización automática con SUNAT */}
      <div className="nx-card nx-card-padded">
        <h3 className="h-display" style={{ margin: '0 0 4px', fontSize: 16 }}>
        ...
        <div className="nx-field">
          ...
          {provider === 'decolecta' && (
            ...
            >
              ...
              Este mismo token se usa también para la <strong>validación de RUC</strong> en
              la sección <strong>Clientes</strong> y en el Wizard de nueva cotización. No
              necesitas configurar otro token — ambos servicios consumen del mismo plan de
              Decolecta (1,000 peticiones mensuales en el plan free).
            </div>
          )}
        </div>
          ...
          >
            <div className="nx-field" style={{ margin: 0, gridColumn: '1 / -1' }}>
              ...
              <input
                ...
                type="text"
                value={apiUrl}
                onChange={(e) => { setApiUrl(e.target.value); setProviderDirty(true); }}
            ...
            <div className="nx-field" style={{ margin: 0 }}>
              ...
              <input
                ...
                type="text"
                value={authHeader}
                onChange={(e) => { setAuthHeader(e.target.value); setProviderDirty(true); }}
                readOnly={provider !== 'custom'}
                style={{ fontFamily: 'var(--font-mono)', fontSize: 12, background: provider !== 'custom' ? 'white' : undefined }}
            ...
            <div className="nx-field" style={{ margin: 0 }}>
              ...
              <input
                ...
                type="text"
                value={authScheme}
                onChange={(e) => { setAuthScheme(e.target.value); setProviderDirty(true); }}
                readOnly={provider !== 'custom'}
                style={{ fontFamily: 'var(--font-mono)', fontSize: 12, background: provider !== 'custom' ? 'white' : undefined }}
            ...
            <div className="nx-field" style={{ margin: 0, gridColumn: '1 / -1' }}>
              ...
              <input
                ...
                type="text"
                value={dateParam}
                onChange={(e) => { setDateParam(e.target.value); setProviderDirty(true); }}
                readOnly={provider !== 'custom'}
                style={{ fontFamily: 'var(--font-mono)', fontSize: 12, background: provider !== 'custom' ? 'white' : undefined }}
                placeholder="date"
              />
        ...
        >
          <input
            type="checkbox"
            checked={autoSync}
            onChange={(e) => { setAutoSync(e.target.checked); setAutoSyncDirty(true); }}
...
// ─── Tab: Limpieza (v2.23) ───
// Solo visible para super_admin. Permite borrar data transaccional (cotizaciones,
// clientes, productos) para dejar la plataforma limpia antes de entregarla a un cliente.
// NO toca: usuarios, organization_settings, SMTP, dominios, integraciones, histórico de TC.
type CleanupAction = 'quotes' | 'clients' | 'products' | 'all';

interface CleanupOption {
  key: CleanupAction;
  title: string;
  icon: string;
  danger: 'low' | 'high' | 'extreme';
  description: string;
  implications: string[];
  resolve: () => Promise<CleanupResult>;
}

function CleanupTab({ onDone }: { onDone: (msg: string) => void }) {
  const [counts, setCounts] = useState<CleanupCounts | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [pending, setPending] = useState<CleanupOption | null>(null);
  ...
  const reload = async () => {
    try {
      setError(null);
      const c = await fetchCleanupCounts();
      setCounts(c);
    } catch (e: any) {
      setError(e?.message || 'No se pudieron cargar los conteos.');
    }
  };
  ...
  const execute = async () => {
    if (!pending) return;
    if (confirmText.trim().toUpperCase() !== 'LIMPIAR') return;
    setRunning(true);
    try {
      const r = await pending.resolve();
      const parts: string[] = [];
      if (r.quotes_deleted) parts.push(`${r.quotes_deleted} cotización(es)`);
      if (r.quote_items_deleted) parts.push(`${r.quote_items_deleted} ítem(s) de cotización`);
      if (r.clients_deleted) parts.push(`${r.clients_deleted} cliente(s)`);
      if (r.products_deleted) parts.push(`${r.products_deleted} producto(s)`);
      if (r.product_modules_deleted) parts.push(`${r.product_modules_deleted} módulo(s)`);
      const summary = parts.length > 0 ? `✓ Eliminados: ${parts.join(', ')}` : '✓ No había data que borrar';
      onDone(summary);
      setPending(null);
      setConfirmText('');
      await reload();
    } catch (e: any) {
      setError(e?.message || 'No se pudo ejecutar la limpieza.');
    } finally {
      setRunning(false);
    }
  };
  ...
  const options: CleanupOption[] = [
    {
      ...
      icon: 'file',
      danger: 'low',
      description: 'Borra todas las cotizaciones (borradores, aceptadas, rechazadas, archivadas) junto con sus ítems y módulos.',
      implications: [
        `${counts.quotes} cotización(es)`,
        `${counts.quote_items} ítem(s) de cotización`,
        `${counts.quote_item_modules} módulo(s) en ítems`,
      ],
      resolve: cleanupQuotes,
    },
    {
      key: 'clients',
      title: 'Clientes',
      icon: 'building',
      danger: 'high',
      description: 'Borra todos los clientes de la plataforma. Como los clientes tienen cotizaciones asociadas, también se eliminarán TODAS las cotizaciones.',
      implications: [
        `${counts.quotes} cotización(es) (dependencia)`,
        `${counts.clients} cliente(s)`,
      ],
      resolve: cleanupClients,
    },
    {
      key: 'products',
      title: 'Productos y módulos',
      icon: 'box',
      danger: 'high',
      description: 'Borra todos los productos y sus módulos. Como las cotizaciones referencian productos, también se eliminarán TODAS las cotizaciones.',
      implications: [
        `${counts.quotes} cotización(es) (dependencia)`,
        `${counts.products} producto(s)`,
        `${counts.product_modules} módulo(s) de productos`,
      ],
      resolve: cleanupProducts,
    },
    {
      key: 'all',
      title: 'RESET TOTAL',
      icon: 'zap',
      danger: 'extreme',
      description: 'Borra TODA la data transaccional de la plataforma: cotizaciones, clientes, productos y sus módulos. Solo se conservan: usuarios, configuración de organización, SMTP, integraciones, dominios permitidos e histórico de tipos de cambio.',
      implications: [
        `${counts.quotes} cotización(es)`,
        `${counts.clients} cliente(s)`,
        `${counts.products} producto(s) + ${counts.product_modules} módulo(s)`,
      ],
      resolve: cleanupAllTransactional,
    },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      >
        ...
        <div>
          <div style={{ fontWeight: 700, marginBottom: 4 }}>Acciones irreversibles</div>
          Esta sección permite borrar data transaccional de la plataforma para dejarla limpia antes
          de entregarla a un cliente. <strong>Las acciones son permanentes y no se pueden deshacer.</strong>{' '}
          Asegúrate de haber hecho un backup si lo necesitas. La configuración de sistema (usuarios,
          organización, SMTP, integraciones, dominios, histórico de TC) no se ve afectada por ninguna acción.
        </div>
      </div>

      {/* 4 cards de acciones */}
      {options.map((opt) => {
        const isExtreme = opt.danger === 'extreme';
        const isEmpty = (
          opt.key === 'quotes' && counts.quotes === 0
        ) || (
          opt.key === 'clients' && counts.clients === 0 && counts.quotes === 0
        ) || (
          opt.key === 'products' && counts.products === 0 && counts.quotes === 0
        ) || (
          opt.key === 'all' && counts.quotes === 0 && counts.clients === 0 && counts.products === 0
        );
        ...
        return (
          <div
            ...
            style={{
              borderLeft: `4px solid ${isExtreme ? '#dc2626' : opt.danger === 'high' ? '#f97316' : '#eab308'}`,
            }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <Icon name={opt.icon as any} size={15} style={{ color: isExtreme ? '#dc2626' : 'var(--ink-600)' }} />
                  <h3
                    className="h-display"
                    style={{ margin: 0, fontSize: 15, color: isExtreme ? '#dc2626' : 'var(--ink-900)' }}
                  >
                    {opt.title}
                  </h3>
                </div>
                <p style={{ margin: '0 0 10px', fontSize: 12.5, color: 'var(--ink-600)', lineHeight: 1.5 }}>
                  {opt.description}
                </p>
                <div style={{ fontSize: 11.5, color: 'var(--ink-500)' }}>
                  <strong style={{ color: 'var(--ink-600)' }}>Se eliminará:</strong>
                  <ul style={{ margin: '4px 0 0 18px', padding: 0 }}>
                    {opt.implications.map((imp, i) => (
                      <li key={i} style={{ marginBottom: 2 }}>{imp}</li>
                    ))}
                  </ul>
                </div>
              </div>
              <button
                className="btn"
                onClick={() => { setPending(opt); setConfirmText(''); setError(null); }}
                disabled={isEmpty}
                title={isEmpty ? 'No hay datos que borrar' : undefined}
                style={{
                  background: isEmpty ? 'var(--ink-100)' : isExtreme ? '#dc2626' : '#f97316',
                  color: isEmpty ? 'var(--ink-400)' : 'white',
      ...
      <Modal open={pending !== null} onClose={() => !running && setPending(null)}>
        {pending && (
          <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
            <h2 className="h-display" style={{ margin: 0, fontSize: 17, color: '#991b1b' }}>
              Confirmar: {pending.title}
            </h2>
            ...
            <div style={{ fontSize: 12.5, color: 'var(--ink-700)' }}>
              <strong>Se eliminará de forma permanente:</strong>
              <ul style={{ margin: '6px 0 0 20px', padding: 0, fontSize: 12.5 }}>
                {pending.implications.map((imp, i) => (
                  <li key={i} style={{ marginBottom: 3 }}>{imp}</li>
                ))}
              </ul>
            </div>

            <div className="nx-field" style={{ margin: 0 }}>
              <label className="nx-label" style={{ fontSize: 12 }}>
                Para confirmar, escribe <strong>LIMPIAR</strong> en el campo:
              </label>
              <input
                className="nx-input"
                type="text"
                ...
                style={{
                  fontFamily: 'var(--font-mono)',
                  fontSize: 13,
                  borderColor: confirmText.trim().toUpperCase() === 'LIMPIAR' ? '#16a34a' : undefined,
                }}
              />
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
              ...
              <button
                className="btn"
                onClick={execute}
                disabled={running || confirmText.trim().toUpperCase() !== 'LIMPIAR'}
                style={{
                  background: confirmText.trim().toUpperCase() === 'LIMPIAR' ? '#dc2626' : 'var(--ink-200)',
                  color: confirmText.trim().toUpperCase() === 'LIMPIAR' ? 'white' : 'var(--ink-400)',
                  cursor: confirmText.trim().toUpperCase() === 'LIMPIAR' ? 'pointer' : 'not-allowed',
                  padding: '8px 14px',
                  fontSize: 12.5,
                  fontWeight: 600,
                  borderRadius: 6,
              ...
              >
                {running ? <div className="spinner" /> : <Icon name="trash" size={13} />}
                Confirmar borrado
              </button>
            </div>
          </div>
...
// ─── Tab: Branding (v2.24) ───
// Solo visible para super_admin. Permite rebrandear toda la plataforma:
// logos, colores, tipografías, nombre comercial y tagline.
// Los cambios se aplican en vivo gracias al BrandingContext.
...
interface ColorField {
  key: 'color_primary' | 'color_secondary' | 'color_ink_dark' | 'color_bg_light' | 'color_success';
  label: string;
  hint: string;
}

const COLOR_FIELDS: ColorField[] = [
  { key: 'color_primary', label: 'Primario', hint: 'Color principal. Botones de acción, enlaces, tab activa.' },
  { key: 'color_secondary', label: 'Secundario', hint: 'Acentos. Hover de botones, íconos destacados, bordes.' },
  { key: 'color_ink_dark', label: 'Texto oscuro', hint: 'Títulos, sidebar y footer oscuro de cotizaciones.' },
  { key: 'color_bg_light', label: 'Fondo claro', hint: 'Fondo general de páginas. Contraste con cards blancas.' },
  { key: 'color_success', label: 'Éxito', hint: 'Cotizaciones aceptadas, confirmaciones, toasts positivos.' },
];
...
function BrandingTab({ onSaved }: { onSaved: () => void }) {
  ...
  const load = async () => {
    try {
      ...
      setError(e?.message || 'No se pudo cargar el branding.');
    }
  };
  ...
  const handleFileUpload = async (
    ...
    field: 'logo_main_url' | 'logo_inverse_url' | 'favicon_url',
  ) => {
    if (!file) return;
    ...
    try {
      ...
      // Auto-guardar la URL para que se refleje inmediatamente sin esperar al "Guardar"
      const updated = await updateBranding({ [field]: url } as any);
      setForm(updated);
      setDirty(false);
  ...
  const handleRemoveAsset = async (
    ...
    if (!confirm('¿Quitar este archivo y volver al valor por defecto?')) return;
    setUploadingSlot(slot);
    try {
      await removeBrandingAsset(slot);
      const updated = await updateBranding({ [field]: null } as any);
      setForm(updated);
      setDirty(false);
      await reloadBranding();
      onSaved();
    } catch (e: any) {
      setError(e?.message || 'No se pudo quitar el archivo.');
    } finally {
      setUploadingSlot(null);
    }
  };
  ...
  const save = async () => {
    ...
    try {
      const updated = await updateBranding({
        ...
        color_ink_dark: form.color_ink_dark,
        color_bg_light: form.color_bg_light,
        color_success: form.color_success,
        font_display: form.font_display,
        font_body: form.font_body,
        font_display_url: form.font_display_url,
        font_body_url: form.font_body_url,
      });
      setForm(updated);
      setDirty(false);
      await reloadBranding();
  ...
  const reset = async () => {
    ...
    setError(null);
    try {
      const restored = await resetBrandingToDefaults();
      // Borrar también los assets del Storage para que no queden huérfanos
      await Promise.all([
        removeBrandingAsset('logo-main').catch(() => null),
        removeBrandingAsset('logo-inverse').catch(() => null),
        removeBrandingAsset('favicon').catch(() => null),
      ]);
      setForm(restored);
      setDirty(false);
      await reloadBranding();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      >
        <Icon name="info" size={14} style={{ flexShrink: 0, marginTop: 2 }} />
        <div>
          <div style={{ fontWeight: 700, marginBottom: 2, color: '#78350F' }}>
            Cambios visibles en toda la plataforma
          </div>
          Afecta el panel, login, PDFs de cotización, emails y link público del cliente. Si no
          tocas nada, se mantienen los valores actuales.
        </div>
      </div>

      {/* Identidad */}
      <div className="nx-card nx-card-padded">
        <h3 className="h-display" style={{ margin: '0 0 2px', fontSize: 16 }}>Identidad</h3>
        <p style={{ margin: '0 0 14px', fontSize: 11.5, color: 'var(--ink-500)' }}>
          Cómo se llama tu empresa en la plataforma. Aparece en el sidebar, login, emails y PDFs.
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div className="nx-field" style={{ margin: 0 }}>
            ...
            <input
              ...
              placeholder="Nexova"
            />
            <div style={{ fontSize: 10.5, color: 'var(--ink-500)', marginTop: 4, lineHeight: 1.4 }}>
              Texto grande del sidebar. Reemplaza "NEXOVA" en todas las pantallas.
            </div>
          </div>
          <div className="nx-field" style={{ margin: 0 }}>
            <label className="nx-label">Tagline <span style={{ color: 'var(--ink-400)', fontWeight: 400 }}>(opcional)</span></label>
            <input
              className="nx-input"
              value={form.tagline || ''}
            ...
            />
            <div style={{ fontSize: 10.5, color: 'var(--ink-500)', marginTop: 4, lineHeight: 1.4 }}>
              Texto pequeño debajo del nombre. Un slogan o descripción breve.
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Logos */}
      <div className="nx-card nx-card-padded">
        <h3 className="h-display" style={{ margin: '0 0 2px', fontSize: 16 }}>Logos</h3>
        <p style={{ margin: '0 0 14px', fontSize: 11.5, color: 'var(--ink-500)' }}>
          SVG recomendado por ser escalable sin pérdida de calidad. PNG y WebP también funcionan. Máximo 5 MB.
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
          <LogoSlot
            label="Logo principal"
            hint="Tu logo a color. Se usa en el sidebar, login y header de PDFs. 512×128 px recomendado."
            currentUrl={form.logo_main_url}
            uploading={uploadingSlot === 'logo-main'}
            onUpload={(f) => handleFileUpload(f, 'logo-main', 'logo_main_url')}
            onRemove={() => handleRemoveAsset('logo-main', 'logo_main_url')}
            bgForPreview={form.color_primary}
          />
          <LogoSlot
            label="Logo inverso"
            hint="Versión blanca o clara, para fondos oscuros como el footer de cotizaciones."
            currentUrl={form.logo_inverse_url}
            uploading={uploadingSlot === 'logo-inverse'}
            onUpload={(f) => handleFileUpload(f, 'logo-inverse', 'logo_inverse_url')}
            onRemove={() => handleRemoveAsset('logo-inverse', 'logo_inverse_url')}
            bgForPreview={form.color_ink_dark}
            optional
          />
          <LogoSlot
            label="Favicon"
            hint="Ícono de la pestaña del navegador. 64×64 px mínimo."
            currentUrl={form.favicon_url}
            uploading={uploadingSlot === 'favicon'}
            onUpload={(f) => handleFileUpload(f, 'favicon', 'favicon_url')}
            onRemove={() => handleRemoveAsset('favicon', 'favicon_url')}
            bgForPreview={form.color_primary}
            compact
          />
        </div>
      ...
      {/* Paleta */}
      <div className="nx-card nx-card-padded">
        <h3 className="h-display" style={{ margin: '0 0 2px', fontSize: 16 }}>Paleta de colores</h3>
        <p style={{ margin: '0 0 14px', fontSize: 11.5, color: 'var(--ink-500)' }}>
          Los rojos de error y amarillos de advertencia mantienen sus colores universales por usabilidad.
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {COLOR_FIELDS.map((cf) => (
            <div
            ...
            >
              <input
                type="color"
                value={form[cf.key]}
                onChange={(e) => update({ [cf.key]: e.target.value } as any)}
                style={{
                  width: 40,
                  height: 36,
                  border: '1px solid var(--ink-200)',
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--ink-900)' }}>{cf.label}</div>
                  <input
                    type="text"
                    value={form[cf.key]}
                    onChange={(e) => {
                      const v = e.target.value;
                      if (/^#[0-9A-Fa-f]{0,6}$/.test(v)) update({ [cf.key]: v } as any);
                    }}
                    style={{
                      fontFamily: 'var(--font-mono)',
                      fontSize: 11,
                ...
                <div style={{ fontSize: 10.5, color: 'var(--ink-500)', marginTop: 2, lineHeight: 1.4 }}>
                  {cf.hint}
                </div>
              </div>
            </div>
      ...
      {/* Tipografía */}
      <div className="nx-card nx-card-padded">
        <h3 className="h-display" style={{ margin: '0 0 2px', fontSize: 16 }}>Tipografía</h3>
        <p style={{ margin: '0 0 14px', fontSize: 11.5, color: 'var(--ink-500)' }}>
          Las letras que usa toda la plataforma. Se cargan desde Google Fonts automáticamente.
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            <label className="nx-label">Para títulos (display)</label>
            <select
              className="nx-input"
              value={form.font_display}
              onChange={(e) => update({ font_display: e.target.value })}
            >
              {GOOGLE_FONTS_CURATED.map((f) => (
                <option key={f} value={f}>{f}</option>
              ))}
            </select>
            <div style={{ fontSize: 10.5, color: 'var(--ink-500)', marginTop: 4, lineHeight: 1.4 }}>
              Títulos grandes, nombres, números destacados, botones.
            </div>
            <div
              style={{
                marginTop: 8,
                ...
                background: 'var(--ink-50)',
                borderRadius: 6,
                fontFamily: `'${form.font_display}', system-ui, sans-serif`,
                fontSize: 20,
                fontWeight: 700,
          ...
          <div>
            <label className="nx-label">Para cuerpo (body)</label>
            <select
              className="nx-input"
            ...
            <div style={{ fontSize: 10.5, color: 'var(--ink-500)', marginTop: 4, lineHeight: 1.4 }}>
              Párrafos, tablas, formularios. Más legible en tamaños chicos.
            </div>
            <div
              style={{
                marginTop: 8,
        ...
        >
          10 opciones curadas de Google Fonts, las más comunes en manuales de marca corporativos.
          Las fuentes se cargan en demanda desde fonts.googleapis.com.
        </div>
      </div>
      ...
      <Modal open={confirmReset} onClose={() => !resetting && setConfirmReset(false)}>
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <h2 className="h-display" style={{ margin: 0, fontSize: 17 }}>
            Restaurar valores de Nexova
          </h2>
          <p style={{ margin: 0, fontSize: 13, color: 'var(--ink-700)', lineHeight: 1.5 }}>
            Esto revierte <strong>todo</strong> al branding original: logos, colores,
            tipografías, nombre comercial y tagline. Los archivos de logo subidos se eliminarán
            del almacenamiento permanentemente.
          </p>
          <div
            style={{
              padding: 10,
          ...
          >
            Útil si la plataforma está por entregarse a otro cliente y querés resetear, o si
            arruinaste la config y querés empezar de cero.
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
            <button
              className="btn btn-ghost"
            ...
            <button
              ...
              disabled={resetting}
            >
              {resetting ? <div className="spinner" /> : <Icon name="layers" size={14} />}
              Sí, restaurar todo
            </button>
          </div>
        </div>
...
// ─── Componente auxiliar: slot de upload de logo ───
function LogoSlot({
  label,
  hint,
  currentUrl,
    ...
    >
      ...
      <div
        style={{
          ...
          background: currentUrl ? 'white' : bgForPreview,
          borderRadius: 6,
          margin: '0 auto 8px',
          display: 'flex',
          alignItems: 'center',
          ...
          fontWeight: 600,
          overflow: 'hidden',
          border: currentUrl ? '1px solid var(--ink-200)' : 'none',
        }}
      >
        {currentUrl ? (
      ...
      <div style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--ink-900)', marginBottom: 2 }}>
        {label}
      </div>
      <div style={{ fontSize: 10, color: 'var(--ink-500)', marginBottom: 8, lineHeight: 1.4 }}>
        {hint}
      </div>
      {currentUrl ? (
        ...
        <label
          ...
          style={{
            fontSize: 10.5,
            color: optional ? 'var(--ink-400)' : 'var(--teal-700)',
            fontWeight: 600,
            cursor: uploading ? 'not-allowed' : 'pointer',
            opacity: uploading ? 0.5 : 1,
          }}
        >
          {uploading ? 'Subiendo...' : optional ? 'Opcional — subir' : 'Subir archivo'}
        </label>
      )}
      <input
        id={inputId}
