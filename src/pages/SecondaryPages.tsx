...
import { fetchOrgSettings, updateOrgSettings, fetchSmtpSettings, updateSmtpSettings, sendEmail } from '../lib/db';
...
function SmtpTab({ onSaved }: { onSaved: () => void }) {
  ...
  const testSmtp = async () => {
    if (!vendor?.email) {
      setTestResult({ ok: false, msg: 'Tu usuario no tiene un email registrado.' });
      return;
    }
    if (!smtp) return;

    // Si el form tiene cambios sin guardar, pedimos guardar primero
    // (porque la Edge Function lee de la DB, no del form local)
    const dirty =
      form.host !== (smtp.host || '') ||
      form.port !== smtp.port ||
    ...
    try {
      ...
      setTestResult({
        ok: true,
        msg: `Email de prueba enviado a ${vendor.email}. Revisa tu bandeja (y la carpeta de spam).`,
      });
    } catch (e: any) {
      setTestResult({
        ok: false,
