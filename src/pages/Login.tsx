...
// Usa supabase.auth.resetPasswordForEmail() — Supabase envía email con link
// que lleva a /reset-password?... con tokens para cambiar la contraseña.
// ═══════════════════════════════════════════════════════════════════════
...
function ForgotPasswordModal({ onClose }: { onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(e?.message || 'No se pudo enviar el email. Inténtalo más tarde.');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        {sent ? (
          <>
            ...
            <p style={{ margin: '0 0 20px', fontSize: 13.5, color: 'var(--ink-600)', lineHeight: 1.5 }}>
              Si <strong>{email}</strong> está registrado, recibirás un email con un link para
          ...
          <>
            ...
            <p style={{ margin: '0 0 18px', fontSize: 13, color: 'var(--ink-600)', lineHeight: 1.5 }}>
              Ingresa tu email corporativo y te enviaremos un link para crear una contraseña nueva.
            </p>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 18 }}>
              ...
              <button
                ...
                style={{ flex: 1 }}
              >
                {submitting ? <div className="spinner" /> : <Icon name="send" size={14} />}
                Enviar link
              </button>
            </div>
          </>
