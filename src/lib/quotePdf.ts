...
import { computeQuoteTotals, fmtDate, fmtMoney, fmtUSD, getRecurringCharges } from './utils';
...
export function generateQuotePdf(
  ...
  for (const it of quote.items || []) {
    ...
    const rightColWidth = 120; // reservado para precios a la derecha
    const descLines = p.description
      ? pdf.splitTextToSize(p.description, contentWidth - rightColWidth)
      : [];
    const modules = (it.module_ids || [])
      .map((mid) => p.modules?.find((x) => x.id === mid))
      .filter((m): m is NonNullable<typeof m> => !!m);

    const itemHeight =
      16 + // nombre + precio soles
      Math.max(descLines.length * 11.5, hasTc ? 12 : 0) + // desc o USD
      modules.length * 12.5 + // bullets
      14; // separador
  ...
  if (recurring.length > 0) {
    ...
    setText(pdf, COLOR.ink500);
    pdf.text(
      'Cargos periódicos posteriores a la implementación. No están incluidos en el total.',
      margin + recPad,
      ry,
    );
    ry += 18;
    ...
    for (let i = 0; i < recurring.length; i++) {
      ...
      // Línea 1: nombre recurrente (izq, bold) · precio en soles (der, bold)
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(10);
      setText(pdf, COLOR.ink900);
      const nameLabel = r.qty > 1 ? `${r.recurring_name}  × ${r.qty}` : r.recurring_name;
      ...
      // Línea 2: nombre del producto (izq, muted) · precio USD (der, muted)
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(8.5);
      setText(pdf, COLOR.ink500);
      pdf.text(r.product_name, margin + recPad, ry);
      if (hasTc) {
        pdf.text(
      ...
      ry += 18; // gap hasta la siguiente fila (permite lugar para el separador)
    }
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    setText(pdf, COLOR.ink400);
    pdf.text(
      `Nexova · ${quote.code}  ·  Generado ${new Date().toLocaleDateString('es-PE', { day: '2-digit', month: 'short', year: 'numeric' })}`,
      margin,
      pageHeight - 20,
    );
