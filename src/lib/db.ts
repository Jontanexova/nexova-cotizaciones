...
export async function fetchCurrentVendor(): Promise<Vendor | null> {
  // IMPORTANTE: NO envolver getSession() en withTimeout. Supabase client usa
  // getSession() internamente para decidir si hacer refresh del token. Si
  // interrumpimos el proceso con un timeout, el estado del auth puede quedar
  // corrupto y causar logouts espurios. getSession() normalmente es muy rápido
  // (lee localStorage) excepto cuando refresca el token, que puede tardar más.
