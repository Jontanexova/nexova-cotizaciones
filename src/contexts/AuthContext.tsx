...
interface AuthContextValue {
  vendor: Vendor | null;
  loading: boolean;
  /** Solo true para role='super_admin' (acceso exclusivo a Dominios permitidos, gestión de super_admins). */
  isSuperAdmin: boolean;
  /** v2.21: true si es super_admin O admin (permisos administrativos generales). */
  isAdmin: boolean;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const changePassword = async (newPassword: string) => {
    ...
    if (error) throw error;
    // Si el vendor tenía must_change_password=true, limpiarlo ahora
    if (vendor?.must_change_password) {
      try {
        await markPasswordChanged();
      } catch (e) {
        console.error('[Auth] mark_password_changed falló:', e);
      }
      await refreshVendor();
    }
  };
  ...
  const isAdmin = vendor?.role === 'super_admin' || vendor?.role === 'admin';

  return (
    <AuthContext.Provider
      value={{ vendor, loading, isSuperAdmin, isAdmin, signIn, signOut, changePassword, refreshVendor }}
    >
      {children}
    </AuthContext.Provider>
  );
